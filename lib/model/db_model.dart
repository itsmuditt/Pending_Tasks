...
class DatabaseConnect {
  ...
  Future<List<Todo>> searchTodo(String keyword) async {
    ...
    List<Map<String, dynamic>> items =
        await db.rawQuery("Select FROM task WHERE title LIKE '%$keyword%'");
  ...
  Future<void> isDone(Todo todo) async {
    final db = await database;
    int val = todo.isChecked ? 1 : 0;
    await db.update('task', {'isChecked': ((val + 1) % 2)},
